...
class TestGridSearch:
    ...
    def test_grid_search_basic(self, mock_run_single_backtest):
        ...
        def mock_result_func(args):
            ...
            return {
                ...
                "slow": slow,
                "sharpe": 1.2
                + (fast / 100.0),  # Different sharpe for different fast values
                "max_dd": -0.05,
                "cagr": 0.15,
                "equity_final": 102000,
                "volatility": 0.12,
    ...
    def test_grid_search_validation(self):
        ...
        with pytest.raises(
            ValueError, match="All fast values must be less than all slow values"
        ):
            grid_search(
                symbol=self.symbol,
                start=self.start,
    ...
    def test_grid_search_sequential_vs_parallel(self, mock_run_single_backtest):
        ...
        assert set(zip(results_seq["fast"], results_seq["slow"])) == set(
            zip(results_par["fast"], results_par["slow"])
        )
...
class TestSaveOptimizationResults:
    ...
    def test_save_optimization_results_creates_directory(self):
        ...
        results_df = pd.DataFrame({"fast": [5], "slow": [20], "sharpe": [1.2]})
...
class TestOptimizationIntegration:
    ...
    def test_optimization_with_real_data_structure(self):
        ...
        with patch(
            "qbacktester.optimize.run_crossover_backtest", return_value=mock_results
        ):
            results_df = grid_search(
                symbol="TEST",
                start="2023-01-01",
