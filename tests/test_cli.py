...
from qbacktester.cli import create_parser, validate_arguments, create_equity_plot, run_backtest_command
...
class TestCLIParser:
    ...
    def test_run_command_required_args(self):
        ...
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', '2020-01-01', 
            '--end', '2020-12-31', '--fast', '10', '--slow', '20'
        ])
    ...
    def test_run_command_all_args(self):
        ...
        parser = create_parser()
        args = parser.parse_args([
            'run', '--symbol', 'SPY', '--start', '2020-01-01', 
            '--end', '2020-12-31', '--fast', '5', '--slow', '15',
            '--cash', '50000', '--fee-bps', '2.0', '--slippage-bps', '1.0',
            '--plot', '--output-dir', 'custom_reports', '--quiet', '--quick'
        ])
...
class TestArgumentValidation:
    ...
    def test_validate_arguments_invalid_dates(self):
        ...
        parser = create_parser()
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', 'invalid-date', 
            '--end', '2020-12-31', '--fast', '10', '--slow', '20'
        ])
    ...
    def test_validate_arguments_start_after_end(self):
        ...
        parser = create_parser()
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', '2020-12-31', 
            '--end', '2020-01-01', '--fast', '10', '--slow', '20'
        ])
    ...
    def test_validate_arguments_fast_greater_than_slow(self):
        ...
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', '2020-01-01', 
            '--end', '2020-12-31', '--fast', '20', '--slow', '10'
        ])
...
class TestEquityPlot:
    ...
    def test_create_equity_plot(self):
        ...
        equity_curve = pd.Series([100000, 100100, 100200, 100150, 100100, 100300, 100250, 100400, 100350, 100500], index=dates)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            create_equity_plot(equity_curve, trades=trades, title="Test Plot", output_dir=tmp_dir)
...
class TestRunBacktestCommand:
    ...
    def test_run_backtest_command_success(self, mock_run_backtest):
        ...
        parser = create_parser()
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', '2023-01-01', 
            '--end', '2023-01-05', '--fast', '3', '--slow', '6'
        ])
    ...
    def test_run_backtest_command_with_plot(self, mock_run_backtest):
        ...
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', '2023-01-01', 
            '--end', '2023-01-05', '--fast', '3', '--slow', '6', '--plot'
        ])
...
class TestCLIIntegration:
    ...
    def test_cli_main_success(self, mock_run_backtest):
        ...
        with patch('sys.argv', ['qbt', 'run', '--symbol', 'AAPL', '--start', '2023-01-01', 
                               '--end', '2023-01-05', '--fast', '3', '--slow', '6']):
            result = main()
            assert result == 0
...
class TestCLIOutput:
    ...
    def test_quiet_mode(self, mock_run_backtest):
        ...
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', '2023-01-01', 
            '--end', '2023-01-05', '--fast', '3', '--slow', '6', '--quiet'
        ])
    ...
    def test_quick_mode(self, mock_run_backtest):
        ...
        args = parser.parse_args([
            'run', '--symbol', 'AAPL', '--start', '2023-01-01', 
            '--end', '2023-01-05', '--fast', '3', '--slow', '6', '--quick'
        ])
