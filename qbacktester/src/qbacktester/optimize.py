...
def _run_single_backtest(args: Tuple[str, str, str, int, int, float, float, float, str]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            ...
            'equity_final': equity_curve['total_equity'].iloc[-1] if not equity_curve.empty else np.nan,
...
def grid_search(
    ...
    if metric not in ['sharpe', 'cagr', 'calmar', 'max_dd']:
        raise ValueError(f"Unsupported metric: {metric}. Must be one of: sharpe, cagr, calmar, max_dd")
...
def print_optimization_summary(
    ...
    print(f"{'Rank':<4} {'Fast':<6} {'Slow':<6} {'Sharpe':<8} {'CAGR':<8} {'MaxDD':<8} {'Calmar':<8} {'HitRate':<8}")
    print(f"{'-'*60}")
    
    for i, (_, row) in enumerate(top_results.iterrows(), 1):
        print(f"{i:<4} {row['fast']:<6} {row['slow']:<6} "
