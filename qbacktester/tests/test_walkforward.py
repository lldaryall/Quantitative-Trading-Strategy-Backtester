...
from qbacktester.walkforward import (
    walkforward_crossover, _generate_walkforward_windows,
    _calculate_parameter_stability, _calculate_performance_consistency,
    print_walkforward_summary, save_walkforward_results
)
...
class TestWalkforwardCrossover:
    ...
    def test_walkforward_crossover_validation(self):
        ...
        # Test invalid years
        with pytest.raises(ValueError, match="In-sample and out-of-sample years must be positive"):
            walkforward_crossover(
                symbol=self.symbol,
                start=self.start,
                end=self.end,
    ...
    @patch('qbacktester.walkforward.grid_search')
    @patch('qbacktester.walkforward.run_crossover_backtest')
    def test_walkforward_crossover_error_handling(self, mock_run_backtest, mock_grid_search):
        ...
        with patch('qbacktester.walkforward._generate_walkforward_windows') as mock_windows:
...
class TestWalkforwardEdgeCases:
    ...
    def test_walkforward_single_window(self):
        ...
        with patch('qbacktester.walkforward.grid_search') as mock_grid_search, \
             patch('qbacktester.walkforward.run_crossover_backtest') as mock_run_backtest, \
             patch('qbacktester.walkforward._generate_walkforward_windows') as mock_windows:
