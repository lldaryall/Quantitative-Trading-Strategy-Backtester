...
def walkforward_crossover(
    ...
    if in_sample_years + out_sample_years > (end_date - start_date).days / 365.25:
        raise ValueError(
            "Total analysis period must be longer than in-sample + out-of-sample years"
        )
    ...
    for i, window in enumerate(windows):
        ...
        try:
            ...
            if verbose:
                print(
                    f"   ✅ Best params: Fast={best_fast}, Slow={best_slow} ({optimization_metric}={best_metric:.4f})"
                )
        ...
        try:
            ...
            if verbose:
                print(
                    f"   📊 OOS Performance: CAGR={oos_metrics['cagr']:.4f}, "
    ...
    if oos_equity_curves:
        ...
        full_equity_curve = full_equity_curve.groupby(
            full_equity_curve.index
    ...
    summary = {
        ...
        "success_rate": (
            len(successful_windows) / len(window_results) if window_results else 0
        ),
        "overall_metrics": overall_metrics,
        "parameter_stability": _calculate_parameter_stability(successful_windows),
        "performance_consistency": _calculate_performance_consistency(
            successful_windows
        ),
    }
...
def _calculate_parameter_stability(successful_windows: List[Dict]) -> Dict[str, float]:
    ...
    return {
        ...
        "fast_cv": (
            np.std(fast_values) / np.mean(fast_values)
            if np.mean(fast_values) > 0
            else 0
        ),
        "slow_mean": np.mean(slow_values),
        "slow_std": np.std(slow_values),
        "slow_cv": (
            np.std(slow_values) / np.mean(slow_values)
            if np.mean(slow_values) > 0
            else 0
        ),
        "parameter_changes": sum(
            1
            for i in range(1, len(fast_values))
            if fast_values[i] != fast_values[i - 1]
            or slow_values[i] != slow_values[i - 1]
        ),
    }
...
def _calculate_performance_consistency(
    ...
    # Extract OOS metrics
    cagr_values = [
        w["oos_metrics"]["cagr"] for w in successful_windows if w["oos_metrics"]
    ]
    sharpe_values = [
        w["oos_metrics"]["sharpe"] for w in successful_windows if w["oos_metrics"]
    ]
    max_dd_values = [
        w["oos_metrics"]["max_drawdown"] for w in successful_windows if w["oos_metrics"]
    ]
    ...
    return {
        ...
        "cagr_cv": (
            np.std(cagr_values) / np.mean(cagr_values)
            if np.mean(cagr_values) > 0
            else 0
        ),
        "sharpe_mean": np.mean(sharpe_values),
        "sharpe_std": np.std(sharpe_values),
        "sharpe_cv": (
            np.std(sharpe_values) / np.mean(sharpe_values)
...
def print_walkforward_summary(results: Dict[str, Any], top_n: int = 5) -> None:
    ...
    if summary["parameter_stability"]:
        ...
        print(f"\n🔧 Parameter Stability:")
        print(
            f"   Fast Window: {stability['fast_mean']:.1f} ± {stability['fast_std']:.1f} (CV: {stability['fast_cv']:.3f})"
        )
        print(
            f"   Slow Window: {stability['slow_mean']:.1f} ± {stability['slow_std']:.1f} (CV: {stability['slow_cv']:.3f})"
        )
    ...
    if summary["performance_consistency"]:
        ...
        print(f"\n📊 Performance Consistency:")
        print(
            f"   CAGR: {consistency['cagr_mean']:.4f} ± {consistency['cagr_std']:.4f}"
        )
        print(
            f"   Sharpe: {consistency['sharpe_mean']:.4f} ± {consistency['sharpe_std']:.4f}"
        )
    ...
    if successful_windows:
        ...
        print(
            f"{'Window':<6} {'Period':<20} {'Fast':<6} {'Slow':<6} {'Sharpe':<8} {'CAGR':<8} {'MaxDD':<8}"
        )
        print(f"{'-'*60}")
        ...
        for i, window in enumerate(sorted_windows[:top_n]):
            if window["oos_metrics"]:
                ...
                print(
                    f"{window['window']:<6} {period:<20} {window['best_fast']:<6} {window['best_slow']:<6} "
...
def save_walkforward_results(
    results: Dict[str, Any], symbol: str, output_dir: str = "reports"
) -> str:
    """
