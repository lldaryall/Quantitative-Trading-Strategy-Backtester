...
from .optimize import grid_search, print_optimization_summary, save_optimization_results
from .plotting import (
    create_drawdown_plot,
    create_equity_plot,
    create_price_signals_plot,
)
from .run import print_backtest_report, print_quick_summary, run_crossover_backtest
...
Examples:
  ...
  qbt run --symbol MSFT --start 2020-01-01 --end 2023-12-31 --fast 5 --slow 20 --cash 50000 --fee-bps 2.0
    ...
    walkforward_parser = subparsers.add_parser(
        "walkforward", help="Perform walk-forward analysis"
    )
    ...
    run_parser.add_argument(
        "--end", "-e", type=str, required=True, help="End date in YYYY-MM-DD format"
    )

    run_parser.add_argument(
        "--fast", "-f", type=int, required=True, help="Fast SMA window period"
    )

    run_parser.add_argument(
        "--slow", "-sl", type=int, required=True, help="Slow SMA window period"
    )
...
def create_comprehensive_plot(
    results: dict, output_dir: str, symbol: str, fast: int, slow: int
) -> None:
    """
    ...
    equity_path = create_equity_plot(
        equity_curve, trades=trades, title=equity_title, output_dir=output_dir
    )
    ...
    drawdown_path = create_drawdown_plot(
        equity_curve, title=drawdown_title, output_dir=output_dir
    )
    ...
    try:
        ...
        if args.plot:
            ...
            create_comprehensive_plot(
                results, str(output_dir), args.symbol.upper(), args.fast, args.slow
            )
...
def run_optimize_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Save results to CSV
        csv_path = save_optimization_results(
            results_df, args.symbol.upper(), args.output_dir
        )
...
def run_walkforward_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Save results
        csv_path = save_walkforward_results(
            results, args.symbol.upper(), args.output_dir
        )
        print(f"\n💾 Results saved to: {csv_path}")
