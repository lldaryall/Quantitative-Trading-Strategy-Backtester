...
def _download_with_retry(
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"Downloading {symbol} data (attempt {attempt + 1}/{max_retries})")
            
            ticker = yf.Ticker(symbol)
            data = ticker.history(start=start, end=end, interval=interval)
            
            if data.empty:
                raise DataError(f"No data returned for symbol '{symbol}' in date range {start} to {end}")
...
def _validate_and_process_data(
    ...
    """
    if data.empty:
        raise DataError(f"No data available for symbol '{symbol}' in date range {start} to {end}")
