...
from .metrics import daily_returns, get_all_metrics
...
def run_crossover_backtest(params: StrategyParams) -> Dict[str, Any]:
    ...
    # Pull data
    print(
        f"📊 Fetching data for {params.symbol} from {params.start} to {params.end}..."
    )
    price_df = get_price_data(params.symbol, params.start, params.end)

    # Build signals
    print(
        f"🔍 Generating signals with {params.fast_window}/{params.slow_window} SMA crossover..."
    )
...
def print_backtest_report(
    ...
    metrics_table.add_row(
        "Sharpe Ratio", format_number(metrics["sharpe"]), "Risk-adjusted return"
    )
    metrics_table.add_row(
        "Sortino Ratio", format_number(metrics["sortino"]), "Downside risk-adjusted"
    )
    metrics_table.add_row(
        "Volatility", format_percentage(metrics["volatility"]), "Annual volatility"
    )
    metrics_table.add_row(
        "Max Drawdown", format_percentage(metrics["max_drawdown"]), "Largest loss"
    )
    metrics_table.add_row(
        "Calmar Ratio", format_number(metrics["calmar"]), "CAGR / |Max DD|"
    )
    metrics_table.add_row(
        "Hit Rate", format_percentage(metrics["hit_rate"]), "Win percentage"
    )
    metrics_table.add_row(
        "Avg Win", format_percentage(metrics["avg_win"]), "Average winning return"
    )
    metrics_table.add_row(
        "Avg Loss", format_percentage(metrics["avg_loss"]), "Average losing return"
    )
    metrics_table.add_row(
        "VaR (5%)", format_percentage(metrics["var_5pct"]), "5% Value at Risk"
    )
    metrics_table.add_row(
        "CVaR (5%)", format_percentage(metrics["cvar_5pct"]), "5% Conditional VaR"
    )
...
def run_multiple_backtests(
    param_list: list[StrategyParams], title: str = "Multiple Backtest Results"
) -> Dict[str, Any]:
    """
