...
def plot_equity(
    equity_curve: pd.Series, title: str = "Portfolio Equity Curve"
) -> Figure:
    """
...
def plot_drawdown(equity_curve: pd.Series, title: str = "Portfolio Drawdown") -> Figure:
    ...
    ax.fill_between(
        equity_curve.index, drawdown.values, 0, color="red", alpha=0.3, label="Drawdown"
    )

    # Plot drawdown line
    ax.plot(
        equity_curve.index, drawdown.values, linewidth=1, color="darkred", alpha=0.8
    )
...
def plot_price_signals(
    ...
    ax2.fill_between(
        signals.index, 0, signals.values, color=signal_colors, alpha=0.3, step="pre"
    )
    ax2.plot(
        signals.index, signals.values, linewidth=1, color="black", drawstyle="steps-pre"
    )
...
def save_figure(fig: Figure, filename: str, output_dir: str = "reports") -> str:
    ...
    # Save figure
    fig.savefig(
        file_path, dpi=300, bbox_inches="tight", facecolor="white", edgecolor="none"
    )
